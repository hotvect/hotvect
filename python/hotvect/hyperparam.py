...
class FeatureSelectionTask:
    ...
    def _build_algorighm_definition(
        self, name_suffix: str, features: List[List[str]]
    ) -> Dict:
    ...
    def advance(
        ...
        for idx, next_feature in enumerate(candidate_features):
            ...
            algorithm_definition = self._build_algorighm_definition(
                name_suffix=name, features=features
            )
            result = self.hotvect.run_all(algorithm_definition)
            results[name] = {
                "features": result["algorithm_definition"]["vectorizer_parameters"][
                    "features"
                ],
