...
description: Copy/paste runbook for `hv algorithm backtest` on a real algorithm repo (local execution)
tags: [agents, runbook, backtest, local]
---

# Runbook: Local backtest (`hv algorithm backtest`)
...
| --- | --- | --- | --- |
| Algorithm repository, git reference, local data, output directory, scratch directory, test date | `hv algorithm backtest` | `meta/<algorithm_id>/<parameter_version>/result.json`, `hv.log`, and `hv.all.log` | Exit code `0` and a result for every requested reference/date |
...
```bash
hv algorithm backtest ... --algorithm-override "$OVERRIDE_JSON"
```
...
```bash
hv algorithm backtest ... --cache /tmp/hotvect-cache --cache-refresh
```
...
`hv algorithm backtest` exits nonzero when any local run or SageMaker submission fails. Treat a nonzero exit as an incomplete
