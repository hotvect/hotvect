...
The browser process also exposes `GET /health`, `GET /api/metadata`, and `POST /predict`. For headless debugging, omit
`--ui` and the UI-specific source and metadata flags. Local artifact mode
