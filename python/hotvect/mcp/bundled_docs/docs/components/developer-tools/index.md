...
| `hv exp` | Inspect algorithms, parameters, slots, experiments, ramp-up history, and online results | Experiment management |
...
## `hv exp`: inspect the control plane

`hv exp` is the read-only EMS inspection CLI. It is useful for understanding which versions are active, checking a
