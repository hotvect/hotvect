...
3. **Optionally assert during tests.** Call `Namespaces.assertCanonical(RequestAttribute.example_request_field_01)` in unit or integration tests to fail fast if someone forgets to register the enum.
...
```java
Namespace vocabNs = Namespaces.declareNamespace("example_request_field_01");
...
Use this checklist when upgrading an existing algorithm or client repository (including `../your-algorithm-repo`).
...
3. **Update builders/extractors.** When a request extractor (e.g., `RequestAttribute.example_request_field_01`) feeds a transformer, confirm the enum was registered. With the new build-time check, unregistered enums cause `StandardRankingTransformer` construction to fail, so tests should surface any missing registrations.
