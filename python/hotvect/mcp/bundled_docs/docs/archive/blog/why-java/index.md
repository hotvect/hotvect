...
Map.Entry<String, Integer> benchmark(String inputJson) throws Exception {
    ...
    String[] fields = {"event_group_a", "event_group_b", "event_group_c"};
        ...
        while (iterationCount < 500000) {
            ...
            for (String field : fields) {
                ...
                for (JsonNode record : actions) {
                    String itemId = record.get("item").get("item_id").textValue();
                    for (int i = 0; i < 8; i++) {
                        String keySegment = itemId.substring(i, 10);
...
def countItems(inputJson, counters):
    ...
    while iterationCount < 500000:
        ...
        for field in ["event_group_a", "event_group_b", "event_group_c"]:
            actions = parsedJson[field]["events"]
            for record in actions:
                itemId = record["item"]["item_id"]
                for i in range(8):
                    keySegment = itemId[i:10]
...
Many companies are investing into frameworks that allow applied scientists to define feature transformations in Python, which are then translated into performant machine code. While a lot of progress has been made, it has not yet succeeded into covering all common use cases reliably.
