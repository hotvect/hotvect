...
description: Read and intentionally mutate a configured Experiment Management Service through Hotvect's typed Python client
...
a configured Experiment Management Service (EMS) endpoint. It is a client library, not an EMS server. Use the
read-only `hv ems` CLI when inspection is enough; use this API only in authorized application or release automation.
...
`hv ems` exposes the inspection subset as JSON and additionally handles configured online-result partitions. Prefer it
...
- `hv ems` is intentionally read-only even though this library also exposes mutation methods.

See [Configuration and experimentation](../../concepts/configuration-and-experimentation/index.md),
[`hv ems`](../cli/index.md#hv-ems), and
