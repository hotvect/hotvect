...
_SAGEMAKER_BACKTEST_RUNBOOK = PromptDef(
    ...
    text=(
        ...
        "- Last test time: `<YYYY-MM-DD>`\n"
        "- Number of runs (days): `<N>`\n"
        "- Results S3 prefix (where SageMaker writes outputs): `<s3://example-bucket/>`\n"
        "\n"
        "## Steps\n"
        ...
        "        --number-of-runs <N>`\n"
        "\n"
        "   Note: for SageMaker runs, prefer `s3://example-bucket` output/scratch dirs.\n"
        "\n"
        "2) Download results locally:\n"
        "   - `hv-ext download-results \\\n"
        "        --s3-base-prefix <s3://example-bucket/> \\\n"
...
_RUN_AND_COMPARE_FEATURE_AUDITS = PromptDef(
    ...
    text=(
        ...
        "        --source-path <source> \\\n"
        "        --dest-path baseline.audit.jsonl --metadata-path baseline.audit.metadata \\\n"
        "        --samples <N>`\n"
        "\n"
        "2) Run treatment audit:\n"
        "   - `hv audit --algorithm-jar <treatment.jar> --algorithm-name <your-algorithm-name> \\\n"
        "        --source-path <source> \\\n"
        "        --dest-path treatment.audit.jsonl --metadata-path treatment.audit.metadata \\\n"
        "        --samples <N>`\n"
        "\n"
        "3) Diff the audit JSONL (prints JSON on stdout; empty means equal):\n"
        "   - `hv-ext compare-jsonl baseline.audit.jsonl treatment.audit.jsonl`\n"
        "\n"
        "## Notes\n"
        "- Use the same `--source-path` and `--samples` for reproducibility.\n"
        "- If the only diffs are key renames, pass a renaming config:\n"
        "  - `hv-ext compare-jsonl baseline.audit.jsonl treatment.audit.jsonl -c field_mappings.json`\n"
        "\n"
        "## Find relevant docs/code\n"
...
_PREDICT_WITH_FEATURE_LOGGING = PromptDef(
    ...
    text=(
        ...
        "        --source-path <source> \\\n"
        "        --dest-path predict.with-features.jsonl --metadata-path predict.with-features.metadata \\\n"
        "        --samples 200`\n"
        "\n"
        "2) Inspect one example's feature audit:\n"
        "   - `head -n 1 predict.with-features.jsonl | jq '.feature_audit'`\n"
        "\n"
        "## Notes\n"
...
_USE_HOTVECT_CACHING = PromptDef(
    ...
    text=(
        ...
        "- `--cache-refresh` forces recompute (still writes fresh artifacts)\n"
        "\n"
        ...
        "- For SageMaker, use `s3://example-bucket` cache prefixes; local paths do not persist.\n"
        "\n"
        "## Find relevant docs/code\n"
...
_REUSE_EXISTING_OUTPUTS = PromptDef(
    ...
    text=(
        ...
        "  --parameter-path <predict.parameters.zip> \\\n"
        "  --source-path <source> \\\n"
        "  --dest-path out.predict.jsonl --metadata-path out.predict.metadata\n"
        "```\n"
        "\n"
...
_PREDICT_SCORE_EQUIVALENCE_TESTING = PromptDef(
    ...
    text=(
        ...
        "        --source-path <source> --dest-path control.audit.jsonl --metadata-path control.audit.metadata \\\n"
        "        --samples <N>`\n"
        "   - `hv audit --algorithm-jar <treatment.jar> --algorithm-name <your-algorithm-name> \\\n"
        "        --source-path <source> --dest-path treatment.audit.jsonl --metadata-path treatment.audit.metadata \\\n"
        "        --samples <N>`\n"
        "   - `hv-ext compare-jsonl control.audit.jsonl treatment.audit.jsonl`\n"
        "\n"
        ...
        "        --parameter-path <predict.parameters.zip> --source-path <source> \\\n"
        "        --dest-path control.predict.jsonl --metadata-path control.predict.metadata \\\n"
        "        --samples <N>`\n"
        "   - `hv predict --algorithm-jar <treatment.jar> --algorithm-name <your-algorithm-name> \\\n"
        "        --parameter-path <predict.parameters.zip> --source-path <source> \\\n"
        "        --dest-path treatment.predict.jsonl --metadata-path treatment.predict.metadata \\\n"
        "        --samples <N>`\n"
        "\n"
        "3) Strictly compare prediction JSONL:\n"
        "   - `hv-ext compare-jsonl control.predict.jsonl treatment.predict.jsonl`\n"
        "\n"
        "## Notes / common footguns\n"
        "- `hv predict` is already ordered; there is no `--ordered` flag.\n"
...
_ONLINE_OFFLINE_PARITY_INVESTIGATION = PromptDef(
    ...
    text=(
        ...
        "        --dest-path replay.predict.jsonl --samples <N>`\n"
        ...
        "        --source-path <live-sample> --dest-path replay.audit.jsonl --samples <N>`\n"
