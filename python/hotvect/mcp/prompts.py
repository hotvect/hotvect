...
_SETUP_CONFIG = PromptDef(
    ...
    text=(
        "## Goal\n"
        "Ensure `hv` works locally and base directories come from `~/.hotvect/config.json`.\n"
        "\n"
        "## Steps\n"
        ...
        "   - `hv config show` (should succeed)\n"
        "   - If missing, run: `hv config init` (interactive)\n"
        "\n"
        ...
        "- Search docs:\n"
        '  - `rg -n "config\\.json|hv config" python/hotvect/mcp/bundled_docs/docs`\n'
...
_QUALITY_REGRESSION_BACKTEST = PromptDef(
    ...
    text=(
        ...
        "   - `hv algorithm backtest --git-reference <baseline_git_ref> --git-reference <treatment_git_ref> \\\n"
        ...
        "   - `hv results ls <path/to/results/meta> --from-date <YYYY-MM-DD> --to-date <YYYY-MM-DD>`\n"
        "\n"
        ...
        '- `rg -n "metrics compare-quality|results ls" python/hotvect/mcp/bundled_docs/docs python/hotvect`\n'
    ),
)
...
_MAJOR_BACKWARD_COMPAT_PREDICT_REGRESSION = PromptDef(
    ...
    text=(
        ...
        "\n"
        "## Step 1 — List all in-use algorithms from EMS (`hv ems`)\n"
        "Run:\n"
        "```bash\n"
        "hv ems algorithm list-in-use > in_use_algorithms.json\n"
        "```\n"
        "\n"
        "Optional (single slot):\n"
        "```bash\n"
        "hv ems algorithm list-in-use --slot-name <slot-name>\n"
        "```\n"
        "\n"
        ...
        "```bash\n"
        "hv qa utils compare-predictions hvX.pred.jsonl hvXplus1.pred.jsonl --score-eps 1e-6 > compare.strict.json\n"
        "```\n"
        "\n"
        "If deterministic tie order is not guaranteed in your stack, also run:\n"
        "```bash\n"
        "hv qa utils compare-predictions hvX.pred.jsonl hvXplus1.pred.jsonl \\\n"
        "  --score-eps 1e-6 \\\n"
        ...
        "- `compare-predictions` status is `passed`.\n"
        ...
        "\n"
        "## Find relevant docs/code\n"
        '- `rg -n "hv ems algorithm list-in-use|compare-predictions|with_parameter|predict parity" python/hotvect/mcp/bundled_docs/docs python/hotvect`\n'
    ),
)
...
_SAGEMAKER_BACKTEST_RUNBOOK = PromptDef(
    ...
    text=(
        ...
        "   - `hv results download <s3://.../> \\\n"
        "        --dest-base-dir <local_results_dir> \\\n"
        "        --from-date <YYYY-MM-DD> \\\n"
        "        --to-date <YYYY-MM-DD>`\n"
        "\n"
        "3) List what was downloaded and what’s usable:\n"
        "   - `hv results ls <local_results_dir>/meta --from-date <YYYY-MM-DD> --to-date <YYYY-MM-DD>`\n"
        "\n"
        ...
        "\n"
        "## Find relevant docs/code\n"
        '- `rg -n "results download|sagemaker|backtest" python/hotvect/mcp/bundled_docs/docs python/hotvect`\n'
    ),
)

_RUN_AND_COMPARE_FEATURE_AUDITS = PromptDef(
    name="run_and_compare_feature_audits",
    description="Run `hv algorithm audit` for baseline and treatment, then compare the JSONL output to spot feature diffs.",
    text=(
        "## Goal\n"
        ...
        "   - `hv algorithm audit --algorithm-jar <baseline.jar> --algorithm-name <your-algorithm-name> \\\n"
        "        --source-path <source> \\\n"
        "        --dest-path baseline.audit --metadata-path baseline.audit.metadata \\\n"
        "        --samples <N>`\n"
        "\n"
        "2) Run treatment audit:\n"
        "   - `hv algorithm audit --algorithm-jar <treatment.jar> --algorithm-name <your-algorithm-name> \\\n"
        "        --source-path <source> \\\n"
        ...
        "\n"
        "## Find relevant docs/code\n"
        '- `rg -n "hv algorithm audit|compare-jsonl|audit" python/hotvect/mcp/bundled_docs/docs python/hotvect`\n'
    ),
)

_PREDICT_WITH_FEATURE_LOGGING = PromptDef(
    name="predict_with_feature_logging",
    description="Debug composite algorithms by running `hv algorithm predict --log-features` and inspecting `feature_audit`.",
    text=(
        "## Goal\n"
        "Run a full `hv algorithm predict` while also logging dependency-level features (useful for composite algorithms).\n"
        "\n"
        "## Inputs (fill in)\n"
        ...
        "- If you only need features (no scores), use `hv algorithm audit` instead.\n"
        "\n"
        "## Find relevant docs/code\n"
...
_USE_HOTVECT_CACHING = PromptDef(
    ...
    text=(
        ...
        "\n"
        "## Best default: enable caching via `hv algorithm backtest --cache`\n"
...
_REUSE_EXISTING_OUTPUTS = PromptDef(
    ...
    text=(
        ...
        "2) **Best-effort**: enable caching via `cache_base_dir` / `hv algorithm backtest --cache` (reuse if present)\n"
        "\n"
        ...
        '        "with_parameter": "s3://example-bucket/hotvect-cache/my-model@1.2.3/last_test_date_2000-06-01/train/predict-parameters.zip"\n'
        "      }\n"
        "    }\n"
        "  }\n"
        "}\n"
        ...
        "- Use `hv algorithm backtest --cache ...` for iterative backtests, or\n"
...
_PERFORMANCE_REGRESSION_CHECK = PromptDef(
    ...
    text=(
        ...
        "   - `hv algorithm performance-test --algorithm-jar <baseline.jar> --algorithm-name <algo> \\\n"
        ...
        "   - `hv metrics compare-system baseline.r1.perf.metadata/metadata.json treatment.r1.perf.metadata/metadata.json`\n"
        "\n"
        ...
        "- If you inject a custom runtime, verify both the imported Python package version and the shell entrypoints used by the pipeline (for example `hv` and `catboost_train`).\n"
...
_PERFORMANCE_INVESTIGATION_RUNBOOK = PromptDef(
    ...
    text=(
        ...
        "Useful tools:\n"
        "- `hv data dependencies inspect` to inspect exact datasets, then `hv data dependencies download` to materialize them\n"
        "- `hv algorithm audit` for feature diffs\n"
        "- `hv algorithm encode` for encoded-data diffs\n"
...
_PREDICT_SCORE_EQUIVALENCE_TESTING = PromptDef(
    ...
    description="Prove two algorithm builds produce identical `hv algorithm predict` output (same params ZIP + same input).",
    text=(
        "## Goal\n"
        "Prove two algorithm JARs produce **exactly the same** `hv algorithm predict` outputs when using the same\n"
        ...
        "   - `hv algorithm audit --algorithm-jar <control.jar> --algorithm-name <your-algorithm-name> \\\n"
        ...
        "\n"
        "2) Run `hv algorithm predict` for control and treatment using the same parameters ZIP:\n"
        "   - `hv algorithm predict --algorithm-jar <control.jar> --algorithm-name <your-algorithm-name> \\\n"
        "        --parameter-path <predict.parameters.zip> --source-path <source> \\\n"
        "        --dest-path control.predict --metadata-path control.predict.metadata \\\n"
        "        --samples <N>`\n"
        "   - `hv algorithm predict --algorithm-jar <treatment.jar> --algorithm-name <your-algorithm-name> \\\n"
        "        --parameter-path <predict.parameters.zip> --source-path <source> \\\n"
        ...
        "- `hv algorithm predict` writes a destination directory. Ordered predict writes `part-00000.jsonl`; unordered predict writes multiple `part-*.jsonl` files.\n"
        ...
        "- `hv algorithm predict` accepts `--ordered` / `--unordered`; unordered is the default, and you should use ordered mode when you need strict row-for-row diffing.\n"
        ...
        "\n"
        "## Find relevant docs/code\n"
        '- `rg -n "predict score equivalence|hv algorithm predict|compare-jsonl" python/hotvect/mcp/bundled_docs/docs python/hotvect`\n'
    ),
)
...
_ORDERED_BACKTEST_WITH_PINNED_PARAMETERS = PromptDef(
    ...
    text=(
        ...
        "\n"
        "This is the right workflow when you already proved `hv algorithm predict` parity with a fixed params ZIP,\n"
        ...
        "   - `hv metrics compare-quality --control <algo>@<control> --treatment <algo>@<treatment> \\\n"
...
_ONLINE_OFFLINE_PARITY_INVESTIGATION = PromptDef(
    ...
    text=(
        ...
        "   - `hv algorithm predict --algorithm-jar <jar> --algorithm-name <algo> \\\n"
        ...
        "   - `hv algorithm audit --algorithm-jar <jar> --algorithm-name <algo> \\\n"
        "        --source-path <live-sample> --dest-path replay.audit --samples <N>`\n"
        "   - `hv algorithm predict --log-features ...` if you need dependency-level feature output.\n"
        "\n"
