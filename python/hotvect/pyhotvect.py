...
class Hotvect:
    def __init__(
        ...
        self.train_data_path = (
            train_data_path if is_iterable(train_data_path) else [train_data_path]
        )
        ...
        # Validation data
        self.validation_data_location = (
            validation_data_path
            if is_iterable(validation_data_path)
            else [validation_data_path]
        )
    ...
    def metadata_path(self) -> str:
        return os.path.join(
            ...
            self.algorithm_definition["algorithm_name"],
            self.run_id,
        )
    ...
    def score_file_path(self) -> str:
        predict_suffix = (
            "prediction.jsonl.gz" if self.enable_gzip else "prediction.jsonl"
        )
    ...
    def encode_parameter_file_path(self) -> str:
        return os.path.join(self.output_path(), "encode.parameters.zip")
    ...
    def _write_algorithm_definition(self) -> str:
        ...
        algorithm_definition_path = os.path.join(
            self.metadata_path(), "algorithm_definition.json"
        )
    ...
    def generate_states(self) -> Dict:
        states = self.algorithm_definition.get("vectorizer_parameters", {}).get(
            "feature_states", {}
        )
        metadata = {}
        for state_name, instruction in states.items():
            metadata_path = os.path.join(
                self.metadata_path(), f"generate-state-{state_name}.json"
            )
            ...
            source_path = os.path.join(
                self.state_source_base_path, instruction["source_name"]
            )
            ...
            if instruction.get("cache"):
                ...
                metadata[state_name] = {"cache": cached}
            else:
    ...
    def package_encode_parameters(self) -> Dict:
        ...
        return {"sources": to_package, "package": encode_parameter_package_location}
    ...
    def package_predict_parameters(self) -> Dict:
        ...
        algo_parameters_path = self._write_data(
            algorithm_parameters, "algorithm_parameters.json"
        )
