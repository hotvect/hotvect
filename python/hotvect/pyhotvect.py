...
class Hotvect:
    ...
    def encoded_data_file_path(self, algorithm_definition: Dict) -> str:
    ...
    def model_file_path(self, algorithm_definition: Dict) -> str:
        return os.path.join(
            self.output_path(algorithm_definition),
            'model.parameter'
        )

    def score_file_path(self, algorithm_definition: Dict) -> str:
        return os.path.join(
            self.output_path(algorithm_definition),
    ...
    def _write_algorithm_definition(self, algorithm_definition: Dict) -> str:
        """Write algorithm definition so that Java can read it"""
        algorithm_definition_path = os.path.join(
            self.metadata_path(algorithm_definition),
    ...
    def clean(self, algorithm_definition) -> None:
        for file in [
            self.encoded_data_file_path(algorithm_definition),
            self.model_file_path(algorithm_definition),
            self.score_file_path(algorithm_definition)
        ]:
            trydelete(file)
    ...
    def run_all(self, algorithm_definition: Dict, clean=True) -> Dict:
        ...
        result['states'] = self.generate_states(algorithm_definition)
        result['package_encode_params'] = self.package_encode_parameters(algorithm_definition)
        result['encode'] = self.encode(algorithm_definition)
        result['train'] = self.train(algorithm_definition)
        result['package_predict_params'] = self.package_predict_parameters(algorithm_definition)
        result['predict'] = self.predict(algorithm_definition)
        result['evaluate'] = self.evaluate(algorithm_definition)
        if clean:
            self.clean_output(algorithm_definition)
        return result

    def _base_command(self, algorithm_definition: Dict, metadata_location: str) -> List[str]:
        ret = [
            'java',
        ...
        ret.extend(['com.eshioji.hotvect.commandline.Main',
                    ...
                    '--algorithm-definition', self._write_algorithm_definition(algorithm_definition),
                    '--meta-data', metadata_location])
        return ret

    def generate_states(self, algorithm_definition: Dict) -> Dict:
        states = algorithm_definition.get('vectorizer_parameters', {}).get('feature_states', {})
        metadata = {}
        for state_name, instruction in states.items():
            metadata_path = os.path.join(self.metadata_path(algorithm_definition), f'generate-state-{state_name}.json')
            trydelete(metadata_path)

            output_path = self.state_output_path(algorithm_definition, state_name)
            ...
            if instruction.get('cache'):
                ...
                cmd = self._base_command(algorithm_definition, metadata_path)
    ...
    def package_encode_parameters(self, algorithm_definition: Dict) -> Dict:
        encode_parameter_package_location = self.encode_parameter_file_path(algorithm_definition)
    ...
    def encode(self, algorithm_definition: Dict) -> Dict:
        metadata_location = os.path.join(self.metadata_path(algorithm_definition), 'encode_metadata.json')
        trydelete(metadata_location)

        encoded_data_location = self.encoded_data_file_path(algorithm_definition)
    ...
    def train(self, algorithm_definition: Dict) -> Dict:
        metadata_location = os.path.join(self.metadata_path(algorithm_definition), 'train_metadata.json')
        trydelete(metadata_location)

        model_location = self.model_file_path(algorithm_definition)
        trydelete(model_location)

        cmd = [
            'vw', self.encoded_data_file_path(algorithm_definition),
            '--readable_model', model_location,
            '--noconstant',
            '--loss_function', 'logistic',
        ]
        train_params = algorithm_definition['training_parameters']
    ...
    def predict(self, algorithm_definition: Dict) -> Dict:
        metadata_location = os.path.join(self.metadata_path(algorithm_definition), 'predict_metadata.json')
        trydelete(metadata_location)

        score_location = self.score_file_path(algorithm_definition)
        ...
        cmd.extend(['--parameters', self.predict_parameter_file_path(algorithm_definition)])
        runshell(cmd)
        return read_json(metadata_location)

    def evaluate(self, algorithm_definition: Dict) -> Dict:
        metadata_location = os.path.join(self.metadata_path(algorithm_definition), 'evaluate_metadata.json')
        trydelete(metadata_location)
    ...
    def clean_output(self, algorithm_definition: Dict) -> None:
        trydelete(self.encoded_data_file_path(algorithm_definition))
