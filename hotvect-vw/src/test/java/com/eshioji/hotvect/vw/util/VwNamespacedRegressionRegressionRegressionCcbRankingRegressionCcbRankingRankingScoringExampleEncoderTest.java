...
class VwNamespacedRegressionRegressionRegressionCcbRankingRegressionCcbRankingRankingScoringExampleEncoderTest {
    private static final ScoringTransformer<DataRecord<TestRawNamespace, RawValue>, TestFeatureNamespace> REGRESSION_TRANSFORMER =
    ...
    void nonBinaryWithoutWeights() {
        ...
        ScoringExample<DataRecord<TestRawNamespace, RawValue>> testRecord = TestRecords.getTestRecord();
...
//    }
    ...
    private void testEncoding(ScoringExample<DataRecord<TestRawNamespace, RawValue>> testRecord, String expected, boolean binary) {
        testEncoding(testRecord, expected, binary, null);
    }


    private void testEncoding(ScoringExample<DataRecord<TestRawNamespace, RawValue>> testRecord, String expected, boolean binary, DoubleUnaryOperator weightFun) {
        VwNamespacedScoringExampleEncoder<DataRecord<TestRawNamespace, RawValue>, TestFeatureNamespace> subject;
        subject = new VwNamespacedScoringExampleEncoder<>(REGRESSION_TRANSFORMER, TestFeatureNamespace.class, binary, weightFun);
