...
public final class JsonInStringSupport {
    ...
    static void mergeOverride(ObjectNode root, ObjectNode override) {
        injectOverriddenVirtualFields(root, override, new InjectionBudget(MAX_TOTAL_JSON_STRING_BYTES), 0);
        JsonUtils.deepMergeJsonNodeWithArrayReplacement(root, override);
    }

    private static void injectOverriddenVirtualFields(
            ObjectNode root, ObjectNode override, InjectionBudget budget, int depth) {
        if (depth > MAX_DEPTH) {
            return;
        }
        for (var field : override.properties()) {
            String key = field.getKey();
            if (key.endsWith(VIRTUAL_SUFFIX)) {
                String baseKey = key.substring(0, key.length() - VIRTUAL_SUFFIX.length());
                JsonNode raw = root.get(baseKey);
                if (raw != null && raw.isTextual()) {
                    // Expand only edited virtual fields: expanding everything would shadow direct string edits.
                    maybeInjectOne(root, baseKey, raw.asText(), budget);
                }
            }
            if (root.get(key) instanceof ObjectNode child && field.getValue() instanceof ObjectNode childOverride) {
                injectOverriddenVirtualFields(child, childOverride, budget, depth + 1);
            }
        }
    }
