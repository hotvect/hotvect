...
public class DoubleFormatUtils {
    ...
    /**
     * Source code adopted from https://stackoverflow.com/a/10554128/234901
     * @param val
     * @param precision
     * @param sb
     */
    public static void format(double val, int precision, StringBuilder sb) {
        ...
        for (int p = precision - 1; p > 0 && fval < tenPow(p); p--) {
            sb.append('0');
        }
        // Remove rightmost zeros from fval
        while (fval % 10 == 0) {
            fval /= 10;
        }
     ...
     *
     * Deprecated due to a known edge case that fails
     *
     ...
     */
    private static int getExponant(double value) {
