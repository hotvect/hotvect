...
import com.hotvect.core.transform.ranking.SharedTransformation;
...
import static com.hotvect.core.util.IdentityCaching.cachedSharedTransformation;
...
class IdentityCachingTest {
    ...
    void performaceIsBetter(){
        ...
        System.out.println(IntStream.range(0, 10000).mapToObj(i -> original.apply(String.valueOf(i / 1000))).reduce((s1, s2) -> s1.length() < s2.length() ? s1: s2).get());
        long lap = System.nanoTime();
        System.out.println(IntStream.range(0, 10000).mapToObj(i -> subject.apply(String.valueOf(i / 1000))).reduce((s1, s2) -> s1.getSingleString().length() < s2.getSingleString().length() ? s1: s2).get().getSingleString());
        long finished = System.nanoTime();
        System.out.println(TimeUnit.NANOSECONDS.toMillis(lap-start) + " vs " + TimeUnit.NANOSECONDS.toMillis(finished - lap));

        Assertions.assertTrue(lap - start > (finished - lap) * 1.1);
    }
}