...
class GreedyRankerTest {
    ...
    void apply(@ForAll("requests") RankingRequest<String, String> request) {
        ...
        skuId_to_score_mapping_is_correct(request.availableActions(), actual);
    ...
    private void skuId_to_score_mapping_is_correct(List<String> availableActions, List<RankingDecision<String>> actual) {
        availableActions.forEach(ca -> {
            var correspondingRankingDecision = actual.stream().filter(rd -> Objects.equals(rd.action(), ca)).toList();
            correspondingRankingDecision.forEach(rd -> {
                Assertions.assertEquals(toMockFeatureFunction.applyAsInt(ca) % 10, correspondingRankingDecision.get(0).score(), 0.000001);
